        ...
        <div class="card-header bg-secondary">
          ...
          <div class="row">
            ...
            if ($content->is_approved == 0) {
                echo '<form method="post" action="'.base_url().'videonews/approveContent/'.$content->item_id.'">
                <button class="btn btn-success" type="submit" name="submit"> Approve</button>
            </form>';
            }
            else{
                echo '<form method="post" action="'.base_url().'videonews/disapproveContent/'.$content->item_id.'">
            ...
            <div class="col text-right">
                <form method="post" action="<?php echo base_url().'videonews/deleteContent/'.$content->item_id; ?>">
  ...
  <div class="row pb-3">
    ...
    <div class="col">
      ...
      <form method="post" enctype="multipart/form-data" id="uploadimage">
        ...
        <div class="py-2">
          <input class="btn btn-danger" type="file" name="ifile" id="ifile">
      ...
      <label for="imagelink">Edit Video :</label>
      <form method="post" enctype="multipart/form-data" id="uploadvideo">
        <div class="border-danger">
          <video width="200" height="200" controls><source src="<?php echo $content->videolink; ?>" type="video/mp4">Your browser does not support the video tag.</video>
        </div>
        <div class="py-2">
          <input class="btn btn-danger" type="file" name="vfile" id="vfile">
          <input class="btn btn-danger btn-lg" type="submit" name="submit">
        </div>
      </form>
...
$('#uploadimage').submit(function(e){
    ...
    $("#loading").show();
         $.ajax({
             url:"<?php echo base_url();?>videonews/editcontentimage/<?php echo $this->uri->segment(3); ?>",
             type:"post",
...
$('#uploadvideo').submit(function(e){
    ...
    $("#loading").show();
         $.ajax({
             url:"<?php echo base_url();?>videonews/editcontentvideo/<?php echo $this->uri->segment(3); ?>",
             type:"post",
             data:new FormData(this),
             processData:false,
             contentType:false,
